...
class ParticleSwampAttack(object):
    def __init__(
        self,
        sess,
        model,
    ...
    def select_best_replacement(self, pos, x_cur, x_orig, target, replace_list):
        ...
        if self.use_lm:
            ...
            pass
        print(new_x_scores)
        print(np.argsort(new_x_scores)[-1])
    ...
    def crossover(self, x1, pop_best, overall_best):
        x_new = x1.copy()
        r = np.random.rand()
        ratio = self.c1 / (self.c1 + self.c2)
        for i in range(len(x1)):
            r = np.random.rand()
            if r < ratio:
                x_new[i] = pop_best[i]
            else:
                x_new[i] = overall_best[i]
        return x_new
    ...
    def attack(self, x_orig, target, max_change=0.4):
        ...
        for i in range(self.max_iters):
            ...
            childs = [
                self.crossover(
                    pop[pop_ranks[i]], best_pop[pop_ranks[i]], pop[top_attack]
                )
...
class GeneticAtackImproved(object):
    def __init__(
        self,
        sess,
        model,
    ...
    def select_best_replacement(self, pos, x_cur, x_orig, target, replace_list):
        ...
        if self.use_lm:
            ...
            orig_tag = self.lm(orig_word)[0][1]
            # print("** ", self.lm(orig_word)[0])
            # print(replace_list)
            for i, w in enumerate(replace_list):
                if w == 0:
                    break
            else:
                i = self.top_n
            replace_list = replace_list[:i]
            tokens = np.array(
                [self.lm(self.dataset.inv_dict[w])[0][1] for w in replace_list]
            )
            # print([self.lm(self.dataset.inv_dict[w])[0] for w in replace_list])
            filtered_index = tokens != orig_tag
            if len(tokens) < self.top_n:
                filtered_index = np.pad(
                    filtered_index, (0, self.top_n - len(tokens)), constant_values=0
                )

            new_x_scores[filtered_index] = -10000000
        else:
            pass
        # print(new_x_scores)
        if np.max(new_x_scores) > 0:
    ...
    def attack(self, x_orig, target, max_change=0.4):
        ...
        tmp = [
            glove_utils.pick_most_similar_words(
                x_orig[i], self.dist_mat, self.top_n, self.delta
            )
            for i in range(x_len)
        ]
        neigbhours_list = [x[0] for x in tmp]
...
class PerturbSentBaseline(object):
    ...
    def perturb(self, x_cur, pos, x_orig, target):
        ...
        replace_list, _ = glove_utils.pick_most_similar_words(
            src_word, self.dist_mat, 8
        )
        print("length of replace list", len(replace_list))
        if len(replace_list) < 8:
            return x_cur
...
class PerturbSentBaseline_Greedy(object):
    def __init__(
        self, sess, model, batch_model, dataset, dist_mat, lm, n1=20, n2=5, use_lm=True
    ):
        self.sess = sess
